...
# Function to get the Cortex Observables
function Get-Observables
{
	...
	$output = Invoke-Process -FilePath $execPath -ArgumentList "$cortexPath --cortex_url $global:CortexURL --cortex_key $global:CortexAPIKey observables"
    Write-Output $output
}
