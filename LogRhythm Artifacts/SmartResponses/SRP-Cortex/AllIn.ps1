...
function Get-All_In
{
	...
	$output = Invoke-Process -FilePath $execPath -ArgumentList "$cortexPath --cortex_url $global:CortexURL --cortex_key $global:CortexAPIKey all_in --observable $Observable --observable_type $ObservableType"
    Write-Output $output
}
