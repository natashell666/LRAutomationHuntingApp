...
function Get-Analyzer_by_Name
{
	...
	$output = Invoke-Process -FilePath $execPath -ArgumentList "$cortexPath --cortex_url $global:CortexURL --cortex_key $global:CortexAPIKey job_name --observable $Observable --observable_type $ObservableType --name $JobName"
    Write-Output $output
}
